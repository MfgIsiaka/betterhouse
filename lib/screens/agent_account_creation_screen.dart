...
class AgentAccountCreationScreen extends StatefulWidget {
 Map<dynamic,dynamic> agent;
  ...
  @override
  State<AgentAccountCreationScreen> createState() => _AgentAccountCreationScreenState();
}
ValueNotifier<bool> locationLoading = ValueNotifier(false);

class _AgentAccountCreationScreenState extends State<AgentAccountCreationScreen> {
...
animateToCurrentLocation()async{ 
 ...
 LatLng? _devLocation;
 setState(() {
   _devLocation=LatLng(dataProvider.deviceLocation["latitude"]!,dataProvider.deviceLocation["longitude"]!);
 ...
 _googleMapController.animateCamera(CameraUpdate.newLatLng(_devLocation!));
  Future.delayed( const Duration(milliseconds:600),(){
      _googleMapController.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: LatLng(_devLocation!.latitude,_devLocation!.longitude),zoom: 15)));
    });
  ...
  Size screenSize=MediaQuery.of(context).size;
    return Scaffold(
      appBar: AppBar(
        ...
        title:const Text("Akaunti ya dalali",style: TextStyle(color: Colors.black),),
      ),
      body: Form(
        key:_formKey,
        child: Container(
          ...
          Expanded(
            child: SingleChildScrollView(
              child: Column(
                ...
                const SizedBox(height:10),
                const Text("Ofisi au makazi yako",style:TextStyle(fontWeight:FontWeight.bold)),
                Container(
                  ...
                  child: Column(
                    children: [
                                   ...
                                   child: AbsorbPointer(
                                     child: Container(
                                      height: 40,
                                       ...
                                       child: Row(children: [
                                        Expanded(child: Text(_selectedRegion==null?"Chagua mkoa":_selectedRegion!.name,overflow: TextOverflow.ellipsis,)),
                              ...
                              child:AbsorbPointer(
                               child: Container(
                                height: 40,
                                 ...
                                 padding: const EdgeInsets.symmetric(horizontal: 5),
                                 child: Row(children: [
                                  Expanded(child: Text(_selectedDistrict==null?"Chagua wilaya":_selectedDistrict!.name,overflow: TextOverflow.ellipsis,)),
                      ...
                      Container(
                        ...
                        child:_showingMap==false?const Center(child: SizedBox(width: 20,height:20, child: CircularProgressIndicator())):GoogleMap(
                        initialCameraPosition: const CameraPosition(target: LatLng(-6.8,39.2833),zoom: 15),
           ...
           ElevatedButton(
            ...
            if(_formKey.currentState!.validate() && _selectedRegion != null && _selectedDistrict != null){
             ...
             if(dataProvider.deviceLocation.isNotEmpty){
               ...
               String res= await DatabaseApi().createAgentProfile(agentInfo);
               if(res=="success"){
                await _usersRef.child(_auth.currentUser!.uid).once().then((value){
                 ...
                 UserReplyWindowsApi().showToastMessage(context,"Jaza taarifa zote.");  
               }
             }else{
                showLocationAlertDialog(dataProvider);
             }
            ...
            },style: ButtonStyle(backgroundColor: MaterialStateColor.resolveWith((states) => kAppColor)), child:const Text("Tuma"))                
          ],),
        )),
    );
  }
   ...
   void showLocationAlertDialog(AppDataProvider provider) {
    ...
    showGeneralDialog(barrierDismissible: false,barrierLabel: "location", context: context,
                              ...
                              children: [
                               const Text(
                                ...
                                ElevatedButton(
                                      ...
                                      child:const Text(
                                      "Nipo eneo husika",style:TextStyle(color: kWhiteColor)))
                                ]);
                    }),
          );
          ...
          if (permission == LocationPermission.always ||
            ...
            await Geolocator.requestPermission().then((locPermission){
              if (permission == LocationPermission.always ||
                ...
                locationLoading.value = false;
                UserReplyWindowsApi().showToastMessage(context, "Ruhusu kifaa kichukue muelekeo");
              }
            });
          }
        });
