...
class ViewedItemsPage extends StatefulWidget {
  const ViewedItemsPage({Key? key}) : super(key: key);

  @override
  State<ViewedItemsPage> createState() => _ViewedItemsPageState();
}

class _ViewedItemsPageState extends State<ViewedItemsPage> {
...
final ScrollController _gridAutoScrController=ScrollController();
 ...
 Future<void> getBuildingsFromDatabase(AppDataProvider dataProvider)async{
   List propertyIds=[];
   //UserReplyWindowsApi().showToastMessage(context,cartItems.toString());
       setState(() {
...
@override
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp)async{
      ...
      cartItems=dataProvider.currentUser["viewedItems"];
      ...
      _gridAutoScrController.addListener(() async {
        if(_gridAutoScrController.hasClients) {
          if(lastPropertyIndex < cartItems.length){
  ...
  Size screenSize=MediaQuery.of(context).size;
    return (dataProvider!.currentUser["viewedItems"] == null || dataProvider!.currentUser["viewedItems"].isEmpty)?const Center(
                        child: Text("Hakuna mali uliyotazama",style:TextStyle(fontWeight: FontWeight.bold)),
                      ):_isLoadingFirstTime == true
